...
function media(bucket,path,type){
  ...
  if(type==="photo")return '<img class="media" loading="lazy" src="'+esc(u)+'" alt="Фото">';
  if(type==="video")return '<div class="media-wrap"><video class="media meeting-video" controls preload="metadata" playsinline src="'+esc(u)+'"></video></div>';
  return '<audio class="media" controls preload="metadata" src="'+esc(u)+'"></audio>';
}
function bindMediaErrors(root=document){
  root.querySelectorAll("img.media").forEach(el=>el.addEventListener("error",()=>{el.style.display="none"},{once:true}));
  root.querySelectorAll("video.media").forEach(el=>el.addEventListener("error",()=>{
    let box=el.closest(".media-wrap")||el.parentElement;
    if(box)box.innerHTML='<div class="notice video-error">Видео не удалось загрузить. Проверьте, что bucket museum-videos открыт для публичного чтения и файл имеет корректный формат.</div>';
  },{once:true}));
  root.querySelectorAll("audio.media").forEach(el=>el.addEventListener("error",()=>{
    el.outerHTML='<div class="notice">Аудио не удалось загрузить.</div>';
  },{once:true}));
}
